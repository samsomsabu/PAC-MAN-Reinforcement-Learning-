...
"""
from utilities.game import GameStateData
from utilities.game import Game
from utilities.game import Directions
from utilities.game import Actions
from utilities.util import nearestPoint
...
def readCommand(argv):
    ...
    if options.quietGraphics:
        from utilities import textDisplay
        ...
        from utilities import graphicsDisplay
...
def loadAgent(pacman, nographics):
    ...
    # Add agents from from agents folder
    pythonPathDirs = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'agents')

    moduleNames = [f for f in os.listdir(pythonPathDirs) if f.endswith('gents.py')]
    for modulename in moduleNames:
        try:
            module = __import__('agents.' + modulename[:-3], fromlist=['*'])
        except ImportError:
            continue
